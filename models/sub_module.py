...
class SPADEResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt):
        ...
        self.norm_0 = SPADE(spade_config_str, fin, opt.input_nc)
        self.norm_1 = SPADE(spade_config_str, fmiddle, opt.input_nc)
        if self.learned_shortcut:
            self.norm_s = SPADE(spade_config_str, fin, opt.input_nc)
...
class SPADEPlusResnetBlock(nn.Module):
    def __init__(self, fin, fout, opt):
        super().__init__()
        # Attributes
        self.learned_shortcut = (fin != fout)   
        ...
        # define normalization layers
        spade_config_str = opt.norm_G.replace('spectral', '')
        self.norm_0 = SPADEPlus(spade_config_str, fin, opt.input_nc)
        self.norm_1 = SPADEPlus(spade_config_str, fmiddle, opt.input_nc)
        if self.learned_shortcut:
            self.norm_s = SPADEPlus(spade_config_str, fin, opt.input_nc)
