...
class SPADEPlusGenerator(BaseNetwork):
    @staticmethod
    def modify_commandline_options(parser, is_train):
        parser.set_defaults(norm_G='spectralspadesyncbatch3x3')
        parser.add_argument('--num_upsampling_layers',
    ...
    def __init__(self, opt):
        ...
        self.head_0 = SPADEPlusResnetBlock(16 * nf, 16 * nf, opt)

        self.G_middle_0 = SPADEPlusResnetBlock(16 * nf, 16 * nf, opt)
        self.G_middle_1 = SPADEPlusResnetBlock(16 * nf, 16 * nf, opt)

        self.up_0 = SPADEPlusResnetBlock(16 * nf, 8 * nf, opt)
        self.up_1 = SPADEPlusResnetBlock(8 * nf, 4 * nf, opt)
        self.up_2 = SPADEPlusResnetBlock(4 * nf, 2 * nf, opt)
        self.up_3 = SPADEPlusResnetBlock(2 * nf, 1 * nf, opt)
...
# spade multi-scale discriminator
...
# pix2pix original discriminator
        ...
        if isinstance(input, list):
            ...
            for pred_i in input:
                if isinstance(pred_i, list):
                    # pick only last output
                    pred_i = pred_i[-1]
                    target_tensor = self.get_target_tensor(pred_i, target_is_real)
                else:
                    target_tensor = self.get_target_tensor(pred_i, target_is_real)
                loss_tensor = self.loss(pred_i, target_tensor)
            ...
            target_tensor = self.get_target_tensor(input, target_is_real)
            loss_tensor = self.loss(input, target_tensor)
            return loss_tensor
...
class FeatLoss(nn.Module):
    def __init__(self, opt):
        ...
        self.GAN_Feat_loss = nn.Parameter(torch.FloatTensor(1).fill_(0))

    def forward(self, pred_fake, pred_real):
        num_D = len(pred_fake)
        GAN_Feat_loss = self.GAN_Feat_loss.clone().detach()
